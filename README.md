...
- Browse all 1,080 kurals with Tamil and English side by side
- Keyboard navigation (← / → arrow keys) and flanking card arrows
...
- **Play All** — plays the current kural first, then shuffles the rest and plays them all in sequence
...
### 3. Gender Neutrality
Thiruvalluvar's philosophy is universal. Where the Tamil uses male-specific or gendered terms as a grammatical convention (a common feature of classical Tamil), this edition uses gender-neutral language — "they/them/a person/a partner" rather than "he/him/wife/husband" — so the wisdom speaks to every reader equally.
...
Audio is available for all **1,080 kurals**. The script to regenerate audio is in `scripts/generate-audio.js` and requires an `.env` file with `AZURE_TTS_KEY` and `AZURE_TTS_REGION`.
...
| File | What it covers |
|---|---|
| `tests/data.test.js` | `kurals.json` integrity — required fields, no duplicates, `tamil` matches `line1 + line2`, `meta.total_kurals` accurate, no invalid Tamil Unicode codepoints, 4+3 word structure per couplet |
...
├── audio/
│   └── {number}.mp3        # Pre-generated TTS audio for all 1,080 kurals
├── screenshots/
